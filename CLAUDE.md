...
| `ImmersiveHero.tsx` | **Current homepage hero** (Jul 2026) — full-viewport raw Three.js scene: a rotating tesseract wireframe (two connected cubes, matching `TesseractMark`'s logo projection) that reacts to cursor position, plus a violet/cyan/amber particle field that expands as the visitor scrolls while the wireframe fades ("shatters"), camera pushing forward the whole time. Canvas is `pointer-events-none` and purely decorative — the actual headline/stats/CTAs are real DOM sitting on top, so SEO and screen readers see a complete hero even if WebGL never renders. Entirely disabled (no canvas mounted at all) under `prefers-reduced-motion`. Replaced `HeroMinimal.tsx` directly on the live homepage. |
| `HeroMinimal.tsx` | **Orphaned as of Jul 2026** — the previous hero (parallax violet bloom via CSS gradient, no WebGL). Kept as a fast rollback path if `ImmersiveHero` needs to come out; not imported anywhere currently. |
